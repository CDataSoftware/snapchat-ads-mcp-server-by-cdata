...
CData JDBC Driver connects to Snapchat Ads by exposing them as relational SQL models.

This server wraps that driver and makes Snapchat Ads data available through a simple MCP interface, so LLMs can retrieve live information by asking natural language questions — no SQL required.
...
2. Download and install the CData JDBC Driver for {source}: [https://www.cdata.com/drivers/snapchatads/download/jdbc](https://www.cdata.com/drivers/snapchatads/download/jdbc/)
3. License the CData JDBC Driver:
    * Navigate to the `lib` folder in the installation directory, typically:
        * (Windows) `C:\Program Files\CData\CData JDBC Driver for Snapchat Ads\`
        * (Mac/Linux) `/Applications/CData JDBC Driver for Snapchat Ads/`
    * Run the command `java -jar cdata.jdbc.snapchatads.jar --license`
    * Enter your name, email, and "TRIAL" (or your license key).
4. Configure your connection to the data source (Salesforce as an example):
    * Run the command `java -jar cdata.jdbc.snapchatads.jar` to open the Connection String utility.
...
5. Create a `.prp` file for your JDBC connection (e.g. `snapchat-ads.prp`) using the following properties and format:
    * **Prefix** - a prefix to be used for the tools exposed
    * **ServerName** - a name for your server
    * **ServerVersion** - a version for your server
    * **DriverPath** - the full path to the JAR file for your JDBC driver
    * **DriverClass** - the name of the JDBC Driver Class (e.g. cdata.jdbc.snapchatads.SnapchatAdsDriver)
